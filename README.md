...
该delete_one()方法的第一个参数是一个查询对象，用于定义要删除的文档。
...
### 删除集合
您可以使用该drop()方法删除在MongoDB中调用的表或集合。
```
#删除“customers”集合：
...
mycol.drop()
```
drop()如果集合成功删除，则该方法返回true;如果集合不存在，则返回false。
## Python MongoDB 更新
### 更新集合
您可以使用该update_one()方法更新MongoDB中调用的记录或文档。

该update_one()方法的第一个参数是一个查询对象，用于定义要更新的文档。
...
```
### 更新多条
要更新符合查询条件的所有文档，请使用该update_many()方法。
```
#更新地址以字母“S”开头的所有文档：
...
## Python MongoDB 限制
### 限制结果
要限制MongoDB中的结果，我们使用该limit() 方法。

该limit()方法采用一个参数，一个数字定义要返回的文档数。
